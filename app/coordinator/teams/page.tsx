    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="mb-6">
        <h1 className="font-headline font-black text-2xl sm:text-3xl italic uppercase tracking-tighter text-[#e4e1e9]">
          District <span className="text-[#ffd700]">Teams</span>
        </h1>
        <p className="text-[#c4c6d0]/60 text-xs sm:text-sm mt-1">{district} District</p>
      </div>
            ...
            <div className="sticky top-0 bg-[#131318] border-b border-[#444650]/20 p-4 sm:p-6">
              <div className="flex justify-between items-start gap-4">
                <div className="flex-1">
                  <h2 className="font-headline font-black text-lg sm:text-xl uppercase tracking-tight text-[#ffd700] break-words">
                    {viewTeam.name}
                  </h2>
                  <p className="text-xs text-[#c4c6d0]/60 mt-1 break-words">
                    {viewTeam.registrationId} • {viewTeam.schoolCollege}
                  </p>
                </div>
            ...
            <div className="p-4 sm:p-6 space-y-5">
              ...
              {viewTeam.status === 'PENDING' && (
                <div className="flex flex-col sm:flex-row gap-3 pt-2 border-t border-[#444650]/20">
                  <button
                    onClick={() => updateStatus(viewTeam.id, 'APPROVED')}
                  ...
                  <button
                    onClick={() => updateStatus(viewTeam.id, 'REJECTED')}
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3 mb-6">
        {['ALL', 'PENDING', 'APPROVED', 'REJECTED'].map(s => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-headline font-black uppercase tracking-widest transition-all ${
              filter === s
                ? 'bg-[#ffd700] text-[#002366]'
                : 'border border-[#444650]/40 text-[#c4c6d0] hover:border-[#ffd700] hover:text-[#ffd700]'
            }`}
          >
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-[#0b0b0f] border-b border-[#444650]/20">
              <tr>
                {['Team', 'Players', 'Payment', 'Status', 'Actions'].map(h => (
            ...
            <tbody className="divide-y divide-[#444650]/10">
              {filtered.map(team => (
                <tr key={team.id} className="hover:bg-[#1c1c21] transition-colors">
                  <td className="px-3 sm:px-4 py-3 sm:py-4">
                    <div className="font-headline font-bold text-[#e4e1e9] text-sm break-words">{team.name}</div>
                    <div className="text-xs text-[#c4c6d0]/50 break-words">{team.registrationId}</div>
                    <div className="text-xs text-[#c4c6d0]/50 break-words">{team.schoolCollege}</div>
                  </td>
                  <td className="px-3 sm:px-4 py-3 sm:py-4">
                  ...
                  <td className="px-3 sm:px-4 py-3 sm:py-4">
                    <span className={`text-[0.55rem] sm:text-[0.6rem] font-headline font-bold uppercase tracking-widest border px-2 py-0.5 whitespace-nowrap ${
                      team.payments?.[0]?.status === 'COMPLETED' ? 'text-emerald-400 border-emerald-400/30' : 'text-[#ffd700] border-[#ffd700]/30'
                    }`}>
                      {team.payments?.[0]?.status || 'PENDING'}
                    </span>
                  </td>
                  <td className="px-3 sm:px-4 py-3 sm:py-4">
                    <span className={`text-[0.55rem] sm:text-[0.6rem] font-headline font-bold uppercase tracking-widest border px-2 py-0.5 whitespace-nowrap ${statusCls[team.status] || statusCls.PENDING}`}>
                      {team.status}
                    </span>
                  </td>
                  <td className="px-3 sm:px-4 py-3 sm:py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <button onClick={() => setViewTeam(team)} className="text-[#c4c6d0]/40 hover:text-[#ffd700] transition-colors p-1" title="View Documents">
                        <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>description</span>
                      </button>
                      {team.status === 'PENDING' && (
                        <>
                          <button onClick={() => updateStatus(team.id, 'APPROVED')} className="text-emerald-400/60 hover:text-emerald-400 transition-colors p-1" title="Approve">
                            <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>check_circle</span>
                          </button>
                          <button onClick={() => updateStatus(team.id, 'REJECTED')} className="text-red-400/60 hover:text-red-400 transition-colors p-1" title="Reject">
                            <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>cancel</span>
                          </button>
                        </>
                      )}
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-[#c4c6d0]/40 font-headline uppercase tracking-widest text-sm">
                    No teams found
                  </td>
                </tr>
              )}
